...
class Gambling extends CI_Controller {
  ...
  public function index()
  {
	...
	$xml = $this->gambling_model->getPinnacleOdds();
...
//	$params['awaySpread']  = (is_int((float)$params['awaySpread']) && (float)$params['awaySpread'] != 0) ? (float)$params['awaySpread']  : 100;
    ...
    switch($params['betType']){
      case 'moneyline':
        $moneylineOut = $this->gambling_model->moneyline($params);
		...
		$return[2] = "$".round($avgProft,2);
		$return[3] = round($calcOdds,2);
		//print_r($return);
        break;
      case 'puckline':
        $pucklineOut =	$this->gambling_model->puckline($params);
        ...
        break;
      case 'gametotals':
        $gametotalsOut = $this->gambling_model->gametotals($params);
...
EOD;
  ...
  function getGames(){
   ...
   // print_r($currentUser->group_id);
    $this->load->model('gambling_model');
    $json = array('code' => 0);
    $arrPost = $this->uri->uri_to_assoc(3);
    $today = date('Y-m-d');
    $yesterday = strtotime('-1 day',strtotime($today));
    $yesterday = date('Y-m-d', $yesterday);
    $tomorrow = strtotime('+1 day',strtotime($today));
