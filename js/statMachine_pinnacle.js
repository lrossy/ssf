...
var options = {
    ...
    exporting: {
        enabled: false,
        filename: 'pinnacleanalysis'
    },

    credits: {
        text: 'SmartSportsFan Bet Analyzer',
        position: {
			align: 'right',
			x: -10,
        ...
        series: {
			...
			events: {
				legendItemClick: function(event) {
                        ...
                        if (this.name == "Bet Online")
						{
							if (visibility == 'visible')
							{
								$("#betOnlineRow").css("display","none");
							}
							else
							{
								$("#betOnlineRow").css("display","table-row");
							}
						}
...
function setupChart1(chk){
    ...
    ajaxReq = $.post(URL_BASEURL+'/'+mainURI+'/compare/'+sport,$('form').serialize(), function(data) {
		dataArr = jQuery.parseJSON(data);
		data = dataArr[0];
		dataBetOnline = dataArr[4];
		//dataBetOnline = dataArr[1];
		$('#cumProf').html(dataArr[1]);
		$('#avgProf').html(dataArr[2]);
		$('#oddsPinnacle').html(dataArr[3]);
		$('#cumProfBetOnline').html(dataArr[5]);
		$('#avgProfBetOnline').html(dataArr[6]);
		$('#oddsBetOnline').html(dataArr[7]);
		
		//hide the table rows for non PinnacleOdds
		$('#betOnlineRow').css("display","none");
		var k = 0;
		var arrPlayers = hc_get_line_vals();
        //console.log(arrPlayers);
		var lines = data.split('\n');
		var linesBetOnline = dataBetOnline.split('\n');
...
//mike edits
		count = 0;
		$.each(linesBetOnline, function(lineNo, line) {
		    var items = line.split(';');
            if (lineNo == 0) {
                $.each(items, function(itemNo, item) {
                   options.xAxis.categories.push(item);
