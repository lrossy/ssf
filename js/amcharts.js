...
}var e=this;this.div=document.getElementById(a);
this.div.style.overflow="hidden";
this.measure();if(this.legend){var d=this.legend.position;
switch(d){case"bottom":this.div.appendChild(this.chartDiv);
...
this.initChart();if(this.tempLabels.length>0){for(var c=this.tempLabels.length-1;
c>=0;c--){var b=this.tempLabels[c];
this.addLabel(b.x,b.y,b.text,b.align,b.size,b.color,b.rotation,b.alpha,b.bold)
...
if(AmCharts.IEversion<9){this.mouseX=window.event.clientX-b;
this.mouseY=window.event.clientY-a
}else{this.mouseX=c.offsetX;
...
h=d+""+b+""+e[g].prefix;
break}}}return h};AmCharts.removeSet=function(a){if(a){a.remove()
...
},draw:function(){if(this.color==this.textShadowColor){this.textShadowColor=null
...
this.set.push(b);if(t+y>o){b.hide()
...
this.set.push(b);if(t+y>x){b.hide()
...
aE=J;aj=B+X/2;ai=J;if(B+C>this.x+this.width+this.columnIndex*n){C=this.x+this.width-B+this.columnIndex*n
}if(B<this.x){C-=this.x-B;
B=this.x}if(B<this.x+this.width+this.columnIndex*n&&C>0){az=new AmCharts.Cuboid(this.container,C,J-ao,n,l,ab,K,this.lineThickness,b,this.lineAlpha,q,aq);
