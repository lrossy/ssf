...
class Scrape{
	...
	function nightly(){
		...
		$this->start($arrTGLW ,$this->playoff_marker, 3);
	...
	function findGames($f=1){
		if($f==2)
		{
			...
			$sql="SELECT right(id,4) as id, date  FROM nhl_schedual WHERE date = '$dateStr'";
		}
		elseif($f==1)
		{
		...
		{
			$dateStr = date("Y-m-d", strtotime("-8 days"));
			$dateStr2 = date("Y-m-d");
			$sql="SELECT right(id,4) as id, date  FROM nhl_schedual WHERE date between '$dateStr' and '$dateStr2'";
			echo $sql;
		}
	...
	function start($arrIds, $playoff_marker='2',$rescrape='0'){
		...
		//loop through all 20082009 season
			...
			foreach($arrIds as $currentID)
			{
				...
				$currentID = $this->leading_zeros($currentID,3);
	...
	function _scrape($gameId,$gameType,$file=0, $boxscore = 0, $gameDate = null){
		...
		$f ='';
			$scrape_url = "http://www.nhl.com/scores/htmlreports/" . $this->SeasonID .'/'. $gameType . $gameId.'.HTM'; 
		}
		else{
		...
		if(!$this->rescape){
			if ($xml = $cache->get($pageID)) {
				echo "Cache Hit for $gameId \n Page : $scrape_url\n";
				// Cache hit !
				// Content is in $data
				return $xml;

			} else { 
				echo "Adding Game $gameId \n Page : $scrape_url\n";
	...
	function getOldPlayer($fname, $lname){
		...
		preg_match_all('#<div style="float: left; margin-right: 10px;">(.*?)</div>#', $xml, $data, PREG_SET_ORDER); 
	...
	function add_new_player($fname, $lname, $team, $position,$fullname,$number){
		...
		echo "Adding player: $fname $lname -- $team -- $NHL_ID\n";
	...
	function get_event_summary_data($GameID,$debug,$file=0){
		...
		$pos2 = strpos($data,$this->frenchCheck);
		
		if($pos2 !== FALSE){
			echo "French\n";
			$data =  iconv("ISO-8859-1", "UTF-8", $data);
			$data = str_replace('FÃ/EV','EV',$data);
			//print_r($data);
		}
		//print_r(mb_detect_encoding($str, 'UTF-8', true));
		$data = str_replace(' width="0px"','',$data);
	...
	function insert_event_data($Visdata,$GameID,$indicator,$team,$teamAgainst){
		...
		foreach($arrVisitorPlayers as $visitor){
		   ...
		   //echo "$event_summary_SQL \n";
		   	if(!$this->debug2){
		   		//$query = $this->db->query($event_summary_SQL);
				$result = mysql_query($event_summary_SQL);
			}
		}
	}
	...
	function get_game_data($GameID,$file=0){
		...
		//echo "$game_summary_SQL \n";
		if(!$this->debug2){
			//$query = $this->db->query($game_summary_SQL);
			$result = mysql_query($game_summary_SQL);
		}
		return true;
	}
			...
			foreach ($arrGAInfoChuncks as $gameGoal){
				...
				//	echo "$goalAssist_SQL \n";
					...
					//$result = $this->db->query($goalAssist_SQL);
						$result = mysql_query($goalAssist_SQL);
					}
					...
					if($gameGoal[6]=='unassisted' || $gameGoal[6]=='Unsuccessful Penalty Shot' || $gameGoal[6]=='Penalty Shot'  || $gameGoal[6]=='' ){
						...
						$assist_number = '2';
							...
							if(!$this->debug2){
								...
								$result = mysql_query($assist_SQL2);
							}
						}
					}
				}
	...
	function get_penalty_data($GameID){
		...
		if($this->SeasonID == '20092010' || $this->SeasonID == '20082009' || $this->SeasonID == '20072008' || $this->SeasonID == '20062007'){
			//echo $this->SeasonID;
	...
	function getNHLSchedual($gameType){
		...
		$feed = substr($feed, strpos($feed, '<table class="data schedTbl">'), strlen($feed)-strpos($feed, '<table class="data schedTbl">')); 
		//print_r($feed);
		preg_match_all('#<tr>(.*?)</tr>#', $feed, $schedual, PREG_SET_ORDER); 
		$i=0;
		foreach ($schedual as $gameday){
			//get date
			preg_match_all('#class="skedStartDateSite">(.*?)</div>#', $gameday[1], $datetime, PREG_SET_ORDER);
			//get Teams
			preg_match_all('#href="javascript:void\(0\);">(.*?)</a>#', $gameday[1], $teams, PREG_SET_ORDER);
			preg_match_all('#class="skedStartTimeEST">(.*?)</div>#', $gameday[1], $startTime, PREG_SET_ORDER);
			preg_match_all('#class="skedDataRow network">(.*?)</div>#', $gameday[1], $network, PREG_SET_ORDER);
			preg_match_all('#preview\?id=201302(.*?)">#', $gameday[1], $gameId, PREG_SET_ORDER);
			preg_match_all('#recap\?id=201302(.*?)">#', $gameday[1], $gameIdrecap, PREG_SET_ORDER);
			preg_match_all('#&quot;sked-3-(.*?)-v&quot#', $gameday[1], $gameIdNew, PREG_SET_ORDER);
			$dateSub = explode('&nbsp;',$datetime[0][1]);
			$arrOut[$i]['date']=date('Y-m-d',strtotime($dateSub[0]));
			...
			$arrOut[$i]['gameIdrecap'] = $gameIdrecap[0][1];
			$arrOut[$i]['homeTeam']= $teams[3][1];
			$num = $this->leading_zeros($arrOut[$i]['gameId'],0);
			$arrOut[$i]['full_id']= '20132014'.$gameType.$num;
		...
		elseif ($nl	> 0){
			foreach($arrOut as $gameDay){
				...
				if (substr($homeTeam,0,4) == "Mont")
					$homeTeam = "Montreal";
				if (substr($awayTeam,0,4) == "Mont")
					$awayTeam = "Montreal";
				
				if (isset($gameDay['gameIdrecap']) && $gameDay['gameIdrecap'] != "")
				{
					echo "we get here@@@ ". $gameDay['gameIdrecap'];;
					$gameID = $gameDay['gameIdrecap'];
					$num = $this->leading_zeros($gameID,0);
					$full_id= '20132014'.$gameType.$num;
				}
				...
				{
					$sql = "Select * from nhl_schedual 
					where date = '$date'
					and home_team like '$homeTeam'
					and away_team like '$awayTeam'";
					...
					if(count($result)>=1)
					{
						while ($row = mysql_fetch_assoc($result)) 
						{
							...
							$sql2 = "Delete from nhl_schedual where id = '$row_id'";
							echo "sql2= ".$sql2;
					...
					{
						$homeTeamQuery = str_replace(".","",$homeTeam);
						$sql = "select team_id from new_team
						where sched like '$homeTeamQuery'";
						...
						echo "date: $date";
						$date_concat = date("Ymd",strtotime($date));
						$full_id = "$date_concat$team_id";
						echo $full_id;
						$sql="REPLACE INTO	nhl_schedual (id, date, time, gametype,tv_nat,home_team, away_team,season,full_id,time2) VALUES ('$full_id', '$date', '$time', '$gameType','$tv_nat','$homeTeam','$awayTeam','$currentSeason','$gameID','$time2')";
						echo "$sql";
						if(!$this->debug2){
		...
		//$alphabet = array ("S");
			...
			//	$splitFeed=(split('<tr>',$splitF2[0])); 
				...
				foreach($playerStep2 as $player){
					...
					$pos		=$player[6];
					$sql="REPLACE INTO	nhl_players (id, fname,lname,team, dob , homecity, pos,fullname) VALUES ('$nhlId', '$fname', '$lname', '$team', '$dob', '$city','$pos','$actFullname')\n";
					print_r($sql);
					if(!$this->debug2){
						$query = $this->db->query($sql);
					}
	...
	function getBoxScoreData($GameID,$gameType,$gameDate){
		...
		print("Hometeam:$homeTeam($this->hometeamId) \n");
		print("Awayteam:$awayTeam($this->awayTeamId) \n");
		print("Status:$gameStatus \n");
		print("Period:$gamePeriod \n");
		print("Date:$gameDate \n");
	...
	function getBS_event_summary_data($GameID,$debug,$file=0){
		...
		foreach ($currentPlayer as &$preInsAway){
			...
			//echo "$PlayerFName, $PlayerLName,$teamId,$NHLID \n";
			...
			if(!$ssf_playerId){
				...
				echo "\n New Player: $PlayerFName, $PlayerLName, teamID: $teamId, nhlID: NHLID \n";
				//insert new player
			}
	...
	function getBS_penalty_data($GameID){
		...
		foreach($currentPen as &$penalty){
			...
			//echo $penalty[2];
			preg_match_all('#-(.*?)min#', $penalty[2], $tmpMin, PREG_SET_ORDER);
			//print_r($tmpMin);
